...
typedef struct odp_dma_seg_t {
	...
	union {
		...
		struct {
			 ...
			 *  allocates all required destination packets and this index field can be
			 *  used to define which packet of the to-be-allocated array of packets is
			 *  used as the destination for this particular destination segment
			 *  descriptor. Same index can be reused across subsequent segments if
			...
			/** Packet length
			 *
			 *  Length of the indexed to-be-allocated packet. In case of index reuse
			 *  across segment descriptors, each new index should have a valid 'pkt_len'
			 *  field set, other descriptor 'pkt_len' fields are ignored for the same
			 *  index. 'pkt_len' of an index cannot be shorter than the total length
			 *  of odp_dma_seg_t::len fields of the segment descriptors using this
			 *  packet index.
			 */
			uint32_t pkt_len;
		};
...
typedef struct odp_dma_transfer_param_t {
	...
	union {
		...
		struct {
			 ...
			 *  #ODP_DMA_FORMAT_PACKET are freed.
			 *
			 ...
			 *  segment descriptors. Index (odp_dma_seg_t::index) field can be used to
			 *  index in odp_dma_result_t::dst_pkt packet array.
			 */
